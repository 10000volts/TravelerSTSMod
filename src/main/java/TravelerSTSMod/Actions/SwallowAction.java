...
public class SwallowAction extends AbstractGameAction {
    private int increaseInkAmount;
    private DamageInfo info;
    private static final float DURATION = 0.1F;

    public SwallowAction(AbstractCreature target, DamageInfo info, int a) {
        this.info = info;
        setValues(target, info);
        this.increaseInkAmount = a;
    ...
    public void update() {
        if (this.duration == 0.1F && this.target != null) {
            ...
            if ((((AbstractMonster)this.target).isDying || this.target.currentHealth <= 0) && !this.target.halfDead &&
                ...
                BookAndQuill.increaseInk(AbstractDungeon.player, this.increaseInkAmount);
            }
